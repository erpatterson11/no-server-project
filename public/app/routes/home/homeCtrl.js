angular.module('weatherApp').controller('homeCtrl', function($scope, $q, mainService, weatherData){
...
$scope.getWeatherData = function() {
  ...
  deferred.promise
    .then((res) => mainService.getWeatherData(res.coords.latitude,res.coords.longitude))
...
// Set variables for function
  ...
  var moveClouds = function() {
      ...
      tlCloudMovement.to(pcLeftLarge, 4000/current.windSpeed, {x: "0vw", repeat: -1, yoyo:true},'initial')
                     .to(pcLeftSmall, 2000/current.windSpeed, {x: "150vw", repeat: -1, yoyo:true},'initial')
                     .to(pcRightLarge, 4000/current.windSpeed, {x: "100vw", repeat: -1, yoyo:true},'initial')
                     .to(pcRightSmall, 1500/current.windSpeed, {x: "250vw", repeat: -1, yoyo:true},'initial')
  }
...
// Show background precipitation clouds if it is raining, snowing, or if the cloud cover is greater than 50%
 ...
 if (!precipCloudsShown) {
    if (current.cloudCover > 0.05 && current.cloudCover < 0.25) {
      ...
      tlHourChange.to(pcLeftSmall, 0.5, {left: "-10vw"}, 'initial')
                  .to(pcRightSmall, 0.5, {right: "-20vw"}, 'initial')
    } else if (current.cloudCover > 0.5 || isItRaining || isItSnowing ) {
      tlHourChange.to(pcLeftLarge, 0.5, {left: "-20vw"},'initial')
        .to(pcLeftSmall, 0.5, {left: "-10vw"}, 'initial')
        .to(pcRightLarge, 0.5, {right: "-20vw"}, 'initial')
        .to(pcRightSmall, 0.5, {right: "-20vw", onComplete: moveClouds()}, 'initial')
      precipCloudsShown = true;
    }
  } else if (!isItRaining && !isItSnowing) {
        if (current.cloudCover < 0.5) {
          tlHourChange.to([pcLeftLarge, pcLeftSmall], 0.5, {left: '-250vw'}, 'initial')
            .to([pcRightSmall, pcRightLarge], 0.5, {right: '-200vw'}, 'initial')
...
// Change sun/moon position and colors of artwork
      ...
      tlHourChange.to(artContainer, 0.2, {ease: Linear.easeNone, backgroundImage: `radial-gradient(circle at ${config[0]}% ${config[1]}%, ${config[2]}`}, 'initial')
        .to(mountains, 0.2, {ease: Linear.easeNone, fill: config[3]}, 'initial')
        .to(mountainAccents, 0.2, {ease: Linear.easeNone, fill: config[4]}, 'initial')
        .to(ground, 0.2, {ease: Linear.easeNone, fill: config[5]}, 'initial')
        .to(groundAccent, 0.2, {ease: Linear.easeNone, fill: config[6]}, 'initial')
        .to(mountainLeft, 0.2, {ease: Linear.easeNone, fill: config[7]}, 'initial')
